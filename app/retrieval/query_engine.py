...
ISBN_CSV_PATH = DATA_DIR / "isbns.csv"
...
def _load_isbn_lookup() -> dict:
    """Maps a PDF filename to its best-known ISBN (ISBN-13 preferred,
    falling back to ISBN-10), from data/isbns.csv if it exists --
    generated by scripts/extract_isbns.py, entirely optional. Read fresh
    on every call rather than cached at import time: the file is tiny
    (one row per book), and re-running extract_isbns.py shouldn't require
    restarting a long-running API process to pick up new results."""
    if not ISBN_CSV_PATH.exists():
        return {}
    lookup = {}
    with open(ISBN_CSV_PATH, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            isbn = row.get("isbn_13") or row.get("isbn_10")
            if isbn:
                lookup[row["file_name"]] = isbn
    return lookup
...
def build_context_and_lookup(session: Session, hits) -> tuple[str, dict]:
    ...
    for the model (this project leans hard on never blending two editions
    of the same book into one answer; an ISBN is about as strong an
    ...
    if isbns_used:
        identifiers = "\n".join(f"- {apa}: ISBN {isbn}" for apa, isbn in isbns_used.items())
        context += (
            "\n\n---\n\nKnown ISBNs for the sources above, for your own "
            "disambiguation only -- these identify exactly which edition "
            "an excerpt came from. Never include an ISBN in your answer "
            "text or inside a citation tag; APA citations don't use them.\n"
            + identifiers
        )
...
def _build_messages(question: str, context: str, history: str) -> list:
    system_prompt = (
        ...
        "If user asks you to continue, then continue your answer from where you left off, do not start over."
        "CITATIONS ARE MANDATORY, DO NOT OMIT THEM. If you do not have a citation for a statement, do not make it up, just say you do not know."
        "CITATION TAGS ARE MANDATORY, DO NOT OMIT THEM. If you do not have a citation for a statement, do not make it up, just say you do not know."
        "Some excerpts are followed by a list of known ISBNs -- these exist only to help you tell apart two similar or same-titled books/editions, never repeat an ISBN in your answer or inside a citation tag."
    )
